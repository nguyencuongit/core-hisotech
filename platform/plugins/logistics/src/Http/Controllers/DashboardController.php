...
class DashboardController extends BaseController
{
    ...
    {
        $this->pageTitle(trans('plugins/logistics::logistics.dashboard'));

        Assets::usingVueJS()
            ->addScriptsDirectly([
                'vendor/core/plugins/ecommerce/libraries/daterangepicker/daterangepicker.js',
                'vendor/core/plugins/logistics/js/report.js',
            ])
            ->addStylesDirectly([
        ...
        [$startDate, $endDate] = LogisticHelper::getDateRangeInReport($request);

        if ($request->ajax()) {
            return $this
                ->httpResponse()->setData(view('plugins/logistics::admin.reports.ajax', compact('widget'))->render());
        }
        return view(
            'plugins/logistics::admin.reports.index',
