...
use Botble\Inventory\Models\SupplierApproval;
...
class SupplierService
{
    ...
    public function create(array $data): Supplier
    {
        return DB::transaction(function () use ($data) {
            ...
            $supplier = $this->supplierRepository->create($this->prepareSupplierData($data));
    ...
    public function update(Supplier $supplier, array $data): Supplier
    {
        return DB::transaction(function () use ($supplier, $data) {
            ...
            $newSensitive = $this->sensitiveSnapshot($supplier->fresh()->load(['banks', 'addresses']));

            if ($oldSensitive !== $newSensitive && $supplier->status === SupplierStatusEnum::ACTIVE) {
                $supplier->update([
                    'requires_reapproval' => true,
                    'status' => SupplierStatusEnum::PENDING_APPROVAL->value,
                ]);

                $this->logApprovalAction($supplier, 'resubmit_for_reapproval', SupplierStatusEnum::ACTIVE->value, SupplierStatusEnum::PENDING_APPROVAL->value, 'Sensitive supplier data changed');
                $this->notifySuperAdminForApproval($supplier);
            }
    ...
    protected function prepareSupplierData(array $data, ?Supplier $supplier = null): array
    {
        ...
        if (
            $status === SupplierStatusEnum::PENDING_APPROVAL->value
            && (! $supplier || ! $supplier->submitted_by)
        ) {
        ...
        return Arr::only($data, [
            'code', 'name', 'type', 'tax_code', 'website', 'note', 'status', 'metadata',
            'created_by', 'submitted_by', 'submitted_at', 'approved_by', 'approved_at', 'approval_note', 'requires_reapproval',
        ]);
    }
